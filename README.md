...
| 6 — V4 hook **scoring** | ⚠️ heuristic | Family-conditional multipliers (feeApr / volume / il / retention) — heuristic weights informed by hook design intent and gated by the pool's regime features (realized vol, toxicity proxy). NOT calibrated against a measured backtest dataset. **Not** an EVM-state replay. The panel surfaces the multipliers as the assumption surface, and the value is wrapped `EMULATED` with an explicit warning. |
...
- Phase 6 hook-scoring multipliers are heuristic family weights informed by hook design intent — NOT calibrated against a measured backtest dataset. An empirical backtest is the AT-2 follow-up.
